...
module F3SourceCheck
  ...
  class Runner
    def initialize
      @app = File.realpath(ENV.fetch("F3_APP_ROOT"))
      @output = File.expand_path(ENV.fetch("F3_VALIDATION_OUTPUT"))
      raise "Fresh external F3 output required" unless @output.start_with?("/private/tmp/www_f3_") && !@output.start_with?(@app + "/") && !File.exist?(@output) && !File.symlink?(@output)
      FileUtils.mkdir_p(@output, mode: 0o700)
      @result = { status: "IN_PROGRESS", started_utc: Time.now.utc.iso8601(6), build_id: ENV.fetch("F3_BUILD_ID"),
    ...
    def sanitize(value)
      ...
      when String then value.gsub(@app, "$REVISED_APP").gsub(File.dirname(@app), "$F3_RUN")
      else value
      end
    end
    ...
    def save(name, value)
      File.open(File.join(@output, name), File::WRONLY | File::CREAT | File::EXCL, 0o600) do |file|
        file.write(JSON.pretty_generate(sanitize(F3APIProbe.json_value(value))) + "\n")
      end
    end
    ...
    def read_frozen(env_key, expected_hash)
      ...
      raise "Reviewed input must be external F3 scratch" unless path.start_with?("/private/tmp/www_f3_")
    ...
    def prepare_inputs
      @oracle = read_frozen("F3_EXPECTATIONS_JSON", ORACLE_SHA256)
      read_frozen("F3_FIELD_CONTRACT_JSON", FIELD_CONTRACT_SHA256)
      @f1_oracle = read_frozen("F3_F1_EXPECTATIONS_JSON", F1_ORACLE_SHA256)
      ...
      @source_dir = File.realpath(ENV.fetch("F3_SOURCE_DIR"))
      raise "Source directory must be external F3 scratch" unless @source_dir.start_with?("/private/tmp/www_f3_")
    ...
    def import_or_continue
      if ENV["F3_IMPORT_EVIDENCE"]
        path = File.realpath(ENV.fetch("F3_IMPORT_EVIDENCE"))
        raise "Prior import proof must be external F3 scratch" unless path.start_with?("/private/tmp/www_f3_")
        previous = JSON.parse(File.read(path))
        assert("prior proof kind/status", ["F3_SIX_SOURCE_IMPORT_PROOF", "PASS"], previous.values_at("kind", "status"))
        ...
        counts = F3APIProbe::DOMAIN_TABLES.to_h { |table| [table, @facts.rows("SELECT count(*) AS count FROM #{table}").first.fetch("count")] }
        ...
        proof = { kind: "F3_SIX_SOURCE_IMPORT_PROOF", status: outcomes == [6, 6, 0, []] ? "PASS" : "FAIL", recorded_utc: Time.now.utc.iso8601(6),
    ...
    def source_to_sql
      ...
      @records.each_with_index do |record, index|
        ...
        save(name, actual)
        differences = F3APIProbe.deep_compare(record.fetch("expected_catalogue_sources"), actual)
    ...
    def request(label, path, expected: nil, html: false)
      ...
      unless html
        if expected.is_a?(Array)
          # F3's bounded default page is prospective behavior. This unchanged
          # six-source experiment has at most six independent SQL expectations,
          # so every existing index/filter expectation fits the default page.
          assert(label + " six-source default page bound", true, expected.length <= 20,
            evidence_class: "F3_index_contract")
          expected_headers = {
            "X-Total-Count" => expected.length.to_s, "X-Page" => "1", "X-Per-Page" => "20",
            "X-Total-Pages" => expected.empty? ? "0" : "1", "Link" => nil
          }
          row[:pagination_headers] = expected_headers.keys.to_h { |name| [name, response.headers[name]] }
          assert(label + " exact default-page metadata and absent navigation", expected_headers, row[:pagination_headers],
            evidence_class: "F3_index_contract")
        end
        comparison = expected.is_a?(Array) ? F3APIProbe.compare_index(expected, actual) : { "diffs" => F3APIProbe.compare_work(expected, actual) }
    ...
    def run_requests
      ...
      diffs = F3APIProbe.compare_work(changed, responses.fetch("63"))
    ...
    def run
      ...
      @facts = F3APIProbe::SQLFacts.new { ActiveRecord::Base.connection }
      import_or_continue
      source_to_sql
      run_requests
...
probe = F3SourceCheck::Runner.new
probe.run
