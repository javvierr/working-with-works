...
module F3APIProbe
  ...
  def self.compare_index(expected, actual)
    ...
    # F3 prospectively fixes title ties with the unique work ID. Direct SQL
    # expectations use the server collation and the same declared order keys.
    if expected_ids != actual_ids
      diffs << { path: "$", reason: "database title/ID ordering differs", expected: expected_ids, actual: actual_ids }
    end
    ...
    if expected_title_order != observed_title_order
      diffs << { path: "$", reason: "database title ordering differs", expected: expected_title_order, actual: observed_title_order }
    end
  ...
  class SQLFacts
    ...
    def index
      rows("SELECT * FROM works ORDER BY works.title ASC, works.id ASC").map { |work| summary(work) }
    end
    ...
    def state
      tables = DOMAIN_TABLES.to_h do |table|
        ...
        [table, { "count" => data.length, "sha256" => Digest::SHA256.hexdigest(JSON.generate(F3APIProbe.canonical(value))), "columns" => columns, "rows" => data }]
      end
      ...
      value.merge("aggregate_sha256" => Digest::SHA256.hexdigest(JSON.generate(F3APIProbe.canonical(value))))
    end
  end
end
