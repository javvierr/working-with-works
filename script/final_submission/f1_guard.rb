...
# Maintained F1 validation guard; external selective copy only. Never creates a database or changes app semantics.
...
module F1Guard
  ...
  def self.read_target!
    target = JSON.parse(File.read(ENV.fetch("F1_TARGET_JSON")))
    ...
    raise Refusal, "Target is not an F1 disposable name" unless target["database"].match?(/\Awww_f1_[a-z0-9_]+\z/)
    host = target.fetch("host")
    raise Refusal, "Target must be the new F1 private socket" unless host.start_with?("/private/tmp/www_f1_pg_") && host.end_with?("/socket")
    raise Refusal, "Target must identify the new F1 data directory" unless target["data_directory"].start_with?("/private/tmp/www_f1_pg_") && target["data_directory"].end_with?("/data")
    ...
    raise Refusal, "Missing external creation evidence file" unless evidence_path.is_a?(String) && evidence_path.start_with?("/private/tmp/www_f1_") && File.file?(evidence_path)
  ...
  def self.boot!
    ...
    app_root = Pathname.new(ENV.fetch("F1_APP_ROOT")).realpath
    raise Refusal, "App must be an external F1 selective copy" unless app_root.to_s.start_with?("/private/tmp/www_f1_")
    raise Refusal, "Execution copy must not have Git metadata" if app_root.join(".git").exist?
    target = read_target!
    guard = Guard.new(target: target, event_path: ENV.fetch("F1_GUARD_LOG"))
    Dir.chdir(app_root)
  ...
  class Guard
    ...
    def initialize(target:, event_path:)
      ...
      @phase = "boot"
      @sequence = 0
      @identity_key = :"f1_guard_inside_identity_#{object_id}"
      log_path = Pathname.new(event_path)
      raise Refusal, "Guard evidence must be external F1 scratch" unless log_path.expand_path.to_s.start_with?("/private/tmp/www_f1_")
    ...
    def start(_name, _id, payload)
      ...
      payload[:f1_guard_sequence] = event.fetch(:sequence)
    end

    def finish(_name, _id, payload)
      return unless payload[:f1_guard_sequence]
      error_class = payload[:exception]&.first
      emit(kind: "sql_finish", start_sequence: payload[:f1_guard_sequence],
    ...
    def run_negative_controls!
      ...
      callbacks = 0
      synthetic = observed.merge("database" => "www_f1_synthetic_wrong_identity")
      pure_refused = false
      begin
      ...
      wrong_expected = @target.merge("database" => "www_f1_deliberately_wrong_expected_identity")
