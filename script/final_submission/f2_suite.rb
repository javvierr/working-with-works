...
result_path = ENV.fetch("F2_SUITE_RESULT_JSON")
raise F2Guard::Refusal, "Suite result already exists" if File.exist?(result_path)
raise F2Guard::Refusal, "Suite evidence must be external F2 scratch" unless File.expand_path(result_path).start_with?("/private/tmp/www_f2_")
...
raise F2Guard::Refusal, "Reviewed Minitest 6.0.6 required" unless Minitest::VERSION == "6.0.6"
...
raise F2Guard::Refusal, "No existing tests found" if paths.empty?
