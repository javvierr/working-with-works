...
require_relative "f2_sql_probe_support"

module F2SourceCheck
  ...
  class Runner
    def initialize
      @app = File.realpath(ENV.fetch("F2_APP_ROOT"))
      @output = File.expand_path(ENV.fetch("F2_VALIDATION_OUTPUT"))
      raise "Fresh external F2 output required" unless @output.start_with?("/private/tmp/www_f2_") && !@output.start_with?(@app + "/") && !File.exist?(@output) && !File.symlink?(@output)
      FileUtils.mkdir_p(@output, mode: 0o700)
      @result = { status: "IN_PROGRESS", started_utc: Time.now.utc.iso8601(6), build_id: ENV.fetch("F2_BUILD_ID"),
    ...
    def sanitize(value)
      ...
      when String then value.gsub(@app, "$REVISED_APP").gsub(File.dirname(@app), "$F2_RUN")
      else value
      end
    end
    ...
    def save(name, value)
      File.open(File.join(@output, name), File::WRONLY | File::CREAT | File::EXCL, 0o600) do |file|
        file.write(JSON.pretty_generate(sanitize(F2APIProbe.json_value(value))) + "\n")
      end
    end
    ...
    def read_frozen(env_key, expected_hash)
      ...
      raise "Reviewed input must be external F2 scratch" unless path.start_with?("/private/tmp/www_f2_")
    ...
    def prepare_inputs
      @oracle = read_frozen("F2_EXPECTATIONS_JSON", ORACLE_SHA256)
      read_frozen("F2_FIELD_CONTRACT_JSON", FIELD_CONTRACT_SHA256)
      @f1_oracle = read_frozen("F2_F1_EXPECTATIONS_JSON", F1_ORACLE_SHA256)
      ...
      @source_dir = File.realpath(ENV.fetch("F2_SOURCE_DIR"))
      raise "Source directory must be external F2 scratch" unless @source_dir.start_with?("/private/tmp/www_f2_")
    ...
    def import_or_continue
      if ENV["F2_IMPORT_EVIDENCE"]
        path = File.realpath(ENV.fetch("F2_IMPORT_EVIDENCE"))
        raise "Prior import proof must be external F2 scratch" unless path.start_with?("/private/tmp/www_f2_")
        previous = JSON.parse(File.read(path))
        assert("prior proof kind/status", ["F2_SIX_SOURCE_IMPORT_PROOF", "PASS"], previous.values_at("kind", "status"))
        ...
        counts = F2APIProbe::DOMAIN_TABLES.to_h { |table| [table, @facts.rows("SELECT count(*) AS count FROM #{table}").first.fetch("count")] }
        ...
        proof = { kind: "F2_SIX_SOURCE_IMPORT_PROOF", status: outcomes == [6, 6, 0, []] ? "PASS" : "FAIL", recorded_utc: Time.now.utc.iso8601(6),
    ...
    def source_to_sql
      ...
      @records.each_with_index do |record, index|
        ...
        save(name, actual)
        differences = F2APIProbe.deep_compare(record.fetch("expected_catalogue_sources"), actual)
    ...
    def request(label, path, expected: nil, html: false)
      ...
      unless html
        comparison = expected.is_a?(Array) ? F2APIProbe.compare_index(expected, actual) : { "diffs" => F2APIProbe.compare_work(expected, actual) }
    ...
    def run_requests
      ...
      diffs = F2APIProbe.compare_work(changed, responses.fetch("63"))
    ...
    def run
      ...
      @facts = F2APIProbe::SQLFacts.new { ActiveRecord::Base.connection }
      import_or_continue
      source_to_sql
      run_requests
...
probe = F2SourceCheck::Runner.new
probe.run
