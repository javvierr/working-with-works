...
guard.with_phase("suite_preflight") do
  guard.run_negative_controls!
  guard.schema_current!
end
result_path = ENV.fetch("F1_SUITE_RESULT_JSON")
raise F1Guard::Refusal, "Suite result already exists" if File.exist?(result_path)
raise F1Guard::Refusal, "Suite evidence must be external F1 scratch" unless File.expand_path(result_path).start_with?("/private/tmp/www_f1_")
...
raise F1Guard::Refusal, "Reviewed Minitest 6.0.6 required" unless Minitest::VERSION == "6.0.6"
...
raise F1Guard::Refusal, "No existing tests found" if paths.empty?
