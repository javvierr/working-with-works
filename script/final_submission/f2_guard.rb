...
# New F2 validation guard adapted from the current reviewed F1 source.
...
module F2Guard
  ...
  def self.read_target!
    descriptor = Pathname.new(ENV.fetch("F2_TARGET_JSON")).realpath
    raise Refusal, "Target descriptor must be external F2 scratch" unless descriptor.to_s.start_with?("/private/tmp/www_f2_")
    ...
    raise Refusal, "Target is not an F2 disposable name" unless target["database"].match?(/\Awww_f2_[a-z0-9_]+\z/)
    host = target.fetch("host")
    raise Refusal, "Target must be the new F2 private socket" unless host.start_with?("/private/tmp/www_f2_pg_") && host.end_with?("/socket")
    raise Refusal, "Target must identify the new F2 data directory" unless target["data_directory"].start_with?("/private/tmp/www_f2_pg_") && target["data_directory"].end_with?("/data")
    ...
    raise Refusal, "Noncanonical F2 instance path" unless instance_root.match?(%r{\A/private/tmp/www_f2_pg_[a-z0-9_]+\z})
    ...
    raise Refusal, "Missing external creation evidence file" unless evidence_path.is_a?(String) && evidence_path.start_with?("/private/tmp/www_f2_") && File.file?(evidence_path)
  ...
  def self.boot!
    ...
    app_root = Pathname.new(ENV.fetch("F2_APP_ROOT")).realpath
    raise Refusal, "App must be an external F2 selective copy" unless app_root.to_s.start_with?("/private/tmp/www_f2_")
    raise Refusal, "Execution copy must not have Git metadata" if app_root.join(".git").exist?
    target = read_target!
    guard = Guard.new(target: target, event_path: ENV.fetch("F2_GUARD_LOG"))
    Dir.chdir(app_root)
  ...
  class Guard
    ...
    def initialize(target:, event_path:)
      ...
      @phase = "boot"
      @sequence = 0
      @identity_key = :"f2_guard_inside_identity_#{object_id}"
      log_path = Pathname.new(event_path)
      raise Refusal, "Guard evidence must be external F2 scratch" unless log_path.expand_path.to_s.start_with?("/private/tmp/www_f2_")
    ...
    def start(_name, _id, payload)
      ...
      payload[:f2_guard_sequence] = event.fetch(:sequence)
    end

    def finish(_name, _id, payload)
      return unless payload[:f2_guard_sequence]
      error_class = payload[:exception]&.first
      emit(kind: "sql_finish", start_sequence: payload[:f2_guard_sequence],
    ...
    def run_negative_controls!
      ...
      callbacks = 0
      synthetic = observed.merge("database" => "www_f2_synthetic_wrong_identity")
      pure_refused = false
      begin
      ...
      wrong_expected = @target.merge("database" => "www_f2_deliberately_wrong_expected_identity")
