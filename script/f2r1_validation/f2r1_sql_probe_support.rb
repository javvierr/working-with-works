...
module F2R1APIProbe
  ...
  class SQLFacts
    ...
    def rows(sql)
      ...
      F2R1APIProbe.json_value(decoded)
    end
    ...
    def state
      tables = DOMAIN_TABLES.to_h do |table|
        ...
        [table, { "count" => data.length, "sha256" => Digest::SHA256.hexdigest(JSON.generate(F2R1APIProbe.canonical(value))), "columns" => columns, "rows" => data }]
      end
      ...
      value.merge("aggregate_sha256" => Digest::SHA256.hexdigest(JSON.generate(F2R1APIProbe.canonical(value))))
    end
  end
end
