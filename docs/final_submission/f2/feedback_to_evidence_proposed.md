...
## 9. Proposed F2 update — 20 September 2026

The complete original seed and F0/F1 proposed observations above are preserved byte for byte. This appendix does not amend the original register or close any feedback item.

| Item | New observed evidence | Proposed status and remaining work |
|---|---|---|
| FB05 | F1 inert-DOCTYPE false positive reproduced and corrected; explicit demo command; direct work-title/classification and latest-attempt regressions preserved. | IMPLEMENTED_AWAITING_EVALUATION; retain earlier rationale and wider source/evaluation limits. |
| FB06 | Three bounded entities, document/local ownership, selected provenance, versioned atomic re-import and minimal existing detail API/HTML pathway. Six source-derived examples reconcile25 descriptions/22 items/24 relations; five actual browser checks. | IMPLEMENTED_AWAITING_EVALUATION; no full expression graph or full-corpus/participant claim. |
| FB07 | Fresh baseline96/781, final133/1,209; independent frozen oracle;1,070 source/SQL/API/HTML checks with21 GETs; genuine F1 upgrade231 checks/36 GETs; semantic rollback and request invariance controls. | IMPLEMENTED_AWAITING_WIDER_EVALUATION; historical matrices immutable, reserved final cases remain uninspected. |
| FB11 | New maintained guarded validation tools, F1-relative review patch and explicit allowlisted review ZIP. Only new F2 instance stopped, with data retained. | PARTLY_IMPLEMENTED_AWAITING_F5; original setup/CI, clean-clone/public-access verification unchanged. |

No FB01–FB13 item is automatically CLOSED. O1 domain/comparator research, O5 participant outcomes, report/video/public-release and later bounded packages retain their existing unresolved status. See F2_checks.json, F2_acceptance_results.csv and the adjacent final archive validation for actual build-specific evidence.
